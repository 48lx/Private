...
export default function HeroGuessPanel({ isOpen, onClose, initialMode }: Props) {
  ...
  const handleGuess = (champion: Champion) => {
    ...
    if (uziMode) {
      ...
      if (isCorrect) giveReward("uzi"); if (isCorrect || newHistory.length >= UZI_MAX) setSolved(true); try { localStorage.setItem("hero-solved-uzi", new Date().toISOString().split("T")[0]); } catch {} try { const gk=getGroupKey(); window.dispatchEvent(new CustomEvent("orb-check",{detail:{groupKey:gk, mode:"uzi"}})); } catch {}
    } else {
