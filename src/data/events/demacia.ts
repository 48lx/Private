...
export const demaciaEvents: GameEvent[] = [
  ...
  // ─── 会传送的街角井盖 ───
  {
    id: "demacia-bandle-manhole",
    region: "demacia",
    type: "fun",
    weight: 7,
    name: "会传送的街角井盖",
    image: "/events/德玛西亚_02.png",
    desc: "你发现德玛西亚街道上的一个井盖不对劲。它时不时闪烁一下紫色的光，每次闪烁后，上面的花纹都会变成不同的班德尔城图案——蘑菇、齿轮、一张正在做鬼脸的约德尔人脸。一个路过的守卫说，这井盖三天前就这样了，踩上去的人会消失几秒再出现，回来的人都说去了一个奇怪的地方。",
    choices: [
      {
        label: "跳上去体验一下",
        success: { tokens: -2, message: "你深吸一口气，站了上去。眼前一花，你发现自己站在一片巨大的蘑菇森林里，脚下是一张巨大的叶子。还没等你反应，眼前又一花，你回到了德玛西亚。虽然只有几秒，但你的手里多了一样东西。" },
      },
      {
        label: "张贴警示标语，提醒路人别踩",
        success: { tokens: 600, attrDelta: { 魅力: 1 }, message: "守卫觉得你很热心，帮你找来了木板和油漆。你写了个「危险，会踩去班德尔城」的标语插在旁边。结果更多人被吸引来了，排着队往上踩。守卫无奈地给了你一笔「误工费」，让你别再帮忙了。" },
      },
      {
        label: "拿出「魔力泉水石」与井盖相互感应",
        hideCheck: { hasItem: "魔力泉水石" },
        check: { hasItem: "魔力泉水石" },
        success: { redirectTo: "__random_bandle__", vitality: 0, message: "泉水石与井盖产生了强烈的共鸣，紫色的光芒笼罩了你……" },
      },
    ],
  },
  // ─── 提莫队长的蘑菇快递 ───
  {
    id: "bandle-teemo-mushroom",
    region: "demacia",
    type: "hero",
    weight: 0,
    ...
    choices: [
      {
        label: "向提莫队长询问这些糖果有什么作用",
        success: { vitality: -2, addTags: ["初尝禁果"], message: "提莫队长非常惊奇，你居然能发现他。他从口袋中掏出一颗糖果给你品尝，你吃下后竟变成了约德尔的体型，持续了一分钟。变回来后你觉得这个体验实在奇妙，但随后便感觉到头晕目眩。提莫说刚刚拿的是试作品，想要完整品就要通过他的考验。话音刚落，你便被井盖传送到了一个不知名的地方。" },
      },
      {
        label: "假装不知道提莫的存在，拿着包裹就跑",
        success: { vitality: -1, message: "跑了没两步，你突然感觉眼前一片漆黑，不知道什么东西从你身前窜过，回过神来后包裹早已不见。" },
      },
    ],
  },
  // ─── 提莫队长的蘑菇快递II ───
  {
    id: "bandle-teemo-mushroom-2",
    region: "demacia",
    type: "hero",
    weight: 0,
    require: { items: ["魔力泉水石"], tags: ["初尝禁果"] },
    name: "提莫队长的蘑菇快递II",
    image: "/events/德玛西亚_02.png",
    desc: "你又来到了提莫队长的「驿站」，这一次提莫似乎正忙得不可开交，没有发现你的存在。",
    choices: [
      {
        label: "帮助提莫整理快递（敏≥18）",
        check: { attrs: { 敏捷: 18 } },
        success: { addItems: ["约德尔变形糖"], attrDelta: { 敏捷: 1 }, message: "提莫对你非常赏识，将一颗完整的果实送给了你。吃下后可以变成约德尔人，跨地区移动不消耗活力。" },
        failure: { vitality: -2, message: "你把提莫的驿站整得一团乱。尴尬地抬头看向提莫时，只见一根针直直地扎进你的脑袋，之后你就什么都不知道了。" },
      },
      {
        label: "跟提莫打声招呼",
        success: { attrDelta: { 智力: 1 }, vitality: -1, addTags: ["中毒"], message: "提莫没有理睬你，你自己一个人在驿站乱逛，感觉见识到了不少不一样的东西。刚离开驿站，突然感觉脚底似乎踩到了什么东西。" },
      },
    ],
  },
  // ─── 迷路的班德尔城外卖员 ───
  {
    ...
    name: "迷路的班德尔城外卖员",
    image: "/events/德玛西亚_01.png",
    desc: "井盖掀开，一个骑着滑板车的约德尔人冲了出来，车筐里放着一个保温箱。他焦急地东张西望，嘴里念叨着「冕卫庄园、冕卫庄园，到底在哪啊」。他看了你一眼，问你知不知道路。",
    choices: [
      {
        label: "给他指路",
        success: { tokens: 300, vitality: 3, message: "你详细指了方向，他连声道谢，从保温箱里掏出一个还热乎的派塞给你。" },
      },
      {
        label: "告诉他走反了",
        success: { attrDelta: { 魅力: -3 }, message: "你瞎指了一个方向。他感激地往诺克萨斯的方向骑走了。你后来听说有个约德尔人在诺克萨斯边境被拦下来，送的外卖是蘑菇披萨，被守卫吃了。" },
      },
      {
        label: "帮他送这单外卖",
        success: { tokens: 500, addItems: ["蘑菇披萨"], message: "你表示可以代劳。他感动得差点哭出来，把保温箱整个给了你，说客户等了三个小时，他不敢去了。" },
      },
    ],
  },
  // ─── 班德尔城噪音投诉处理员 ───
  {
    ...
    name: "班德尔城噪音投诉处理员",
    image: "/events/德玛西亚_03.png",
    desc: "井盖掀开一条缝，一个戴着耳塞的约德尔人探出头，一脸疲惫。他说他是班德尔城噪音管理局的，最近德玛西亚这边的井盖老传出怪声，影响班德尔城居民的午睡质量。他问你能不能帮忙查查声源。",
    choices: [
      {
        label: "帮他听听是什么声音",
        success: { tokens: 400, vitality: 1, message: "你趴在地上听了一会儿，发现是风吹过井盖缝隙的呼啸声。他松了一口气，说不是怪物就好。" },
      },
      {
        label: "告诉他那是加里奥的心跳声",
        success: { tokens: 300, attrDelta: { 魅力: -1 }, message: "你随口编了个浪漫的说法。他惊恐地睁大眼睛，在报告上写「德玛西亚地下有巨型生物，心跳可传到班德尔城」。这份报告后来引发了一场短暂的恐慌。" },
      },
      {
        label: "建议他给井盖加个消音垫",
        success: { addItems: ["消音垫"], message: "他觉得这个建议非常专业，从工具箱里翻出一个多余的消音垫递给你，说以物易物，他不欠人情。" },
      },
    ],
  },
  // ─── 叽叽的谢礼 ───
  {
    ...
    type: "fun",
    weight: 5,
    require: { items: ["魔力泉水石", "叽叽的口哨"] },
    name: "叽叽的谢礼",
    image: "/events/德玛西亚_02.png",
    desc: "一声尖锐的哨音从空中传来。你抬头，看到当初被你从石像鬼诅咒中解救的约德尔人叽叽，正骑着一只巨大的彩色蜻蜓朝你飞来。他降落在你面前，激动地抓着你的手说：「朋友！我回班德尔城安顿好了，没想到在这里见到你。救命之恩不能不报！」他翻了翻口袋，掏出几样东西让你选一样当谢礼。",
    choices: [
      {
        label: "挑一件有用的",
        success: { addItems: ["附魔口哨套"], message: "你挑了一件看起来最不起眼的小东西——一个皮质的口哨套。叽叽说这是他亲手做的，能保护口哨不被禁魔石干扰，在德玛西亚也能随时吹响。" },
      },
      {
        label: "让他请一顿饭就行",
        success: { vitality: 6, message: "叽叽大笑，说他知道有一家非常出名的酒馆。亲自带你去了附近最好的酒馆，点了一桌他认为是人类最爱吃的菜。" },
      },
      {
        label: "说不用了，当初只是顺手帮忙",
        success: { message: "叽叽眼眶红了，说你果然是好人。但约德尔人不懂什么是客套，所以也没有坚持。" },
      },
    ],
  },
  // ─── 波比的寻找之旅 ───
  {
    ...
    name: "波比的寻找之旅",
    image: "/events/德玛西亚_01.png",
    desc: "一个约德尔人从井中出来，扛着一把比自己还高的锤子，走向街上一脸认真地询问每一个路人：「你好，请问你见过一个需要英雄的人吗？我在找那个人。」",
    choices: [
      {
        label: "「我就是你需要找的人」",
        success: { tokens: 150, vitality: 3, message: "波比认真打量你，然后摇头：「你不像。但你很有趣，我们可以做朋友。」她高兴地和你分享了她带的干粮，食物意外地好吃。" },
      },
      {
        label: "「你知道禁魔石之心吗？」",
        success: { addClues: [{ region: "demacia", type: "D", data: "加里奥" }], message: "波比眼睛一亮，说她当然知道。她最好的朋友——一个叫加里奥的巨像——就和禁魔石之心有关。波比说加里奥平时在睡觉，只有魔法特别浓郁的时候才会醒，还说他醒来后第一句话通常是「波比在哪」。" },
      },
      {
        label: "「我能和你一起成为英雄吗」（力/智/敏≥12）",
        check: { attrs: { 力量: 12, 智力: 12, 敏捷: 12 } },
        success: { addCards: ["gold_德莱厄斯_灌篮高手"], message: "波比在你身旁绕了一圈，突然朝你扔了个圆盾，你机敏地弹开了圆盾并迅速反抢。波比对你的反应十分赞赏，送了你一张她珍藏的卡片，据说是前往诺克萨斯的侦察兵拍下的珍贵资料。" },
        failure: { vitality: -2, message: "波比两下假动作把你唬得两腿发软，她大笑两声说道「你说不定是需要英雄的人哦」。" },
      },
    ],
  },
  // ─── 菲奥娜的剑术课 ───
